...
int main()
{
    ...
    { auto &ov = input; auto on = "input"; cout << on << "[" << ov.size() << "]={"; for_each(ov.begin(), ov.end(), [](const int item) { static int count = 0;  cout << (count++ ? "," : "") << item; }); cout << "}" << endl; }

    vector<unsigned> result;
    find_skipped(input, result);
    { auto &ov = result; auto on = "skip"; cout << on << "[" << ov.size() << "]={"; for_each(ov.begin(), ov.end(), [](const int item) { static int count = 0;  cout << (count++ ? "," : "") << item; }); cout << "}" << endl; }

    { auto &ov = input; auto on = "modified_input"; cout << on << "[" << ov.size() << "]={"; for_each(ov.begin(), ov.end(), [](const int item) { static int count = 0;  cout << (count++ ? "," : "") << item; }); cout << "}" << endl; }
}
